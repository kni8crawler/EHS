...
classdef multiface1 < handle     %class dwfinition
    properties
                ...
                boxIdx = this.findMatchingBox(bboxes(i, :));
                  if isempty(boxIdx)
                   ...
                   region_borders = edge(region_borders, 'Sobel');
                    points2 = detectHarrisFeatures(region_borders, 'ROI', bboxes(i, :));
                    ...
                    while(ii<=size(points2,1) && jj>=1)
                        midrow(ii,:) = (points2(ii).Location + points2(jj).Location)/2;
                         ii=ii+1;
                         jj=jj-1;
                    end
                   ...
                   points4 = detectMinEigenFeatures(region_borders2, 'ROI', bboxes(i, :));
                    region_borders2 = edge(region_borders2, 'Sobel');
                     points3=detectHarrisFeatures(region_borders2, 'ROI', bboxes(i, :));
